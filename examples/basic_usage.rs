...
fn main() {
    ...
    let send_req = SendRequest {
        ...
        client_seq: 1,
        client_msg_no: "msg_001".to_string(),
    ...
    let subscribe_req = SubscribeRequest {
        setting: 1,
        client_msg_no: "sub_001".to_string(),
    ...
    for i in 1..=3 {
        let mut msg = RecvRequest::new();
        msg.message_id = BigInt::from(i);
        msg.from_uid = format!("user_{}", i);
        msg.payload = format!("批量消息 {}", i).as_bytes().to_vec();
        batch_messages.push(msg);
    }
    
    let batch_req = RecvBatchRequest::single_batch(batch_messages);
